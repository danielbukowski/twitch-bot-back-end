...
export default class ObjectManager {
    ...
    public constructor(private configInitializer: ConfigInitializer) {
        ...
        this.manageableClasses.set(AuthManager.name, 
            new AuthManager(
        ...
        this.manageableClasses.set(TwitchChat.name,
            new TwitchChat(
        ...
        this.manageableClasses.set(HttpServer.name, 
            new HttpServer(config.httpServerPort)
        );
        this.manageableClasses.set(SocketClient.name, 
            new SocketClient(
                (this.manageableClasses.get(HttpServer.name) as HttpServer).getHttpServer()
            )
        );
        this.manageableClasses.set(TwitchEventListener.name, 
